...
TAR_FILE := $(TAR_DIR).tar.xz

help:
	@echo "Usage:"
	@echo "  make install - Install firmware files to /usr/lib/firmware/scarlett4"
...
install:
	install -d $(DESTDIR)/usr/lib/firmware/scarlett4
	install -m 644 $(FIRMWARE_FILES) $(DESTDIR)/usr/lib/firmware/scarlett4

tar: $(FIRMWARE_DIR) $(LICENSE_FILES) $(SPEC_FILE)
	mkdir -p $(TAR_DIR)
	cp -r $(FIRMWARE_DIR) $(LICENSE_FILES) $(SPEC_FILE) debian Makefile $(TAR_DIR)/
	tar cJf $(TAR_FILE) $(TAR_DIR)
	rm -rf $(TAR_DIR)
...
prepare_deb: tar
	mkdir -p debian/DEBIAN debian/usr/lib/firmware/scarlett4
	cp $(FIRMWARE_FILES) $(LICENSE_FILES) debian/usr/lib/firmware/scarlett4
